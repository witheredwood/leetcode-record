 ...
 * 714. 买卖股票的最佳时机含手续费
 * <p>
 * 给定一个整数数组 prices，其中第i个元素代表了第i天的股票价格 ；整数 fee 代表了交易股票的手续费用。
...
public class code_0714 {
     ...
     * <p>
     * 买入时加上手续费
     * 卖出股票并且获得 prices[i] - buy 的收益。
     * 卖出时可以看成，当前手上拥有一支买入价格为  prices[i] 的股票，以提供一个反悔操作
     *
     * @param prices 某段时间内的股票价格
     * @param fee    每次交易的手续费
     * @return 该段时间内的最大利润
     */
    public int maxProfit31(int[] prices, int fee) {
        int buy = prices[0] + fee, profit = 0;
        for (int i = 0; i < prices.length; i++) {
            if (prices[i] + fee < buy) {    // 重新买入
                buy = prices[i] + fee;
