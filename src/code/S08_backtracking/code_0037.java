...
public class code_0037 {
     ...
     * @param board 整数矩阵
     */
    public void solveSudoku31(char[][] board) {
        // 找到要填的位置
        List<int[]> emptyList = new LinkedList<>();
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[0].length; j++) {
                if (board[i][j] == '.') emptyList.add(new int[]{i, j});
            }
        }
        backtracking(board, 0, emptyList);
    }

    /**
     * @param board     整数矩阵
     * @param index     要处理的位置的下标
     * @param emptyList 存放要填位置的链表
     * @return 是否完成
     */
    private boolean backtracking(char[][] board, int index, List<int[]> emptyList) {
        if (index == emptyList.size()) {
            return true;
        }
        int row = emptyList.get(index)[0], col = emptyList.get(index)[1];
        boolean isFinished = false;
        for (int i = 1; i <= 9; i++) {
            char cur = (char) ('0' + i);
            boolean isRepeat = false;
            // 同一列
            for (int r = 0; !isRepeat && r < board.length; r++) {
                if (cur == board[r][col]) isRepeat = true;
            }
            // 同一行
            for (int c = 0; !isRepeat && c < board.length; c++) {
                if (cur == board[row][c]) isRepeat = true;
            }
            // 同一块
            int s1 = row / 3 * 3, s2 = col / 3 * 3;
            for (int r = s1; !isRepeat && r < s1 + 3; r++) {
                for (int c = s2; !isRepeat && c < s2 + 3; c++) {
                    if (cur == board[r][c]) isRepeat = true;
                }
            }
            if (!isRepeat) {
                board[row][col] = cur;
                isFinished = backtracking(board, index + 1, emptyList);
                if (!isFinished) board[row][col] = '.';
            }
        }
        return isFinished;
    }
